...
from arm64nsd.domain.errors import InputValidationError, SourceAccessError
from arm64nsd.domain.model import SourceUnit, SourceUnitId
from arm64nsd.domain.ports import SourceRepository
...
class FileSystemSourceRepository(SourceRepository):
    def load_file(self, path: str) -> SourceUnit:
        ...
        if source_path.suffix not in {".S", ".s", ".asm"}:
            raise InputValidationError(f"expected an ARM64 assembly file (.S, .s, or .asm), got: {source_path}")

        return self._load_source_unit(source_path)

    def list_asm_sources(self, root_path: str) -> tuple[SourceUnit, ...]:
        ...
        source_paths = tuple(
            sorted(path for path in root.rglob("*") if path.is_file() and path.suffix in asm_suffixes)
        )
        if not source_paths:
            raise InputValidationError(f"no ARM64 assembly files (.S, .s, .asm) found under: {root}")
